...
`<lhs> = <rhs>`:: Tests that `<lhs>` equals `<rhs>`
`<lhs> ~= <rhs>`:: Tests that `<lhs>` contains a match of the case-insensitive `<rhs>` regex. The command fails if `<lhs>` is not a string

// =========================================================== Examples
include::./util/conditional-examples-header.adoc[]

Test if the application ID is Chrome's: `aerospace test %{app-bundle-id} = com.google.Chrome`
