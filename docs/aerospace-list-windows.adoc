...
--workspace <workspace>...::
Filter results to only print windows that belong to any of the specified workspaces.
...
. `visible` is a special workspace name that represents all currently visible workspaces (In a multi-monitor setup, there are multiple visible workspaces)

include::util/monitor-option.adoc[]

--pid <pid>:: Filter results to only print windows that belong to the application with the specified `<pid>`

--app-bundle-id <app-bundle-id>::
Filter results to only print windows that belong to the application with the specified https://developer.apple.com/documentation/appstoreconnectapi/bundle_ids[Bundle ID]
+
...
Can be used in combination with `--format` to specify which data to include in the JSON.
Incompatible with `--count`
...
%{window-is-fullscreen}:: Boolean. Whether the window is in fullscreen mode (as set by the `aerospace fullscreen` command)
...
%{right-padding}:: A special variable which expands to the minimum number of spaces required to form a right padding in the appropriate column
