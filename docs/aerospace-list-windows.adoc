...
--layout <layout>::
Filter results to only print windows that match the specified `<layout>`.
A tiling window is matched by the layout of its parent container (the one that `%{window-parent-container-layout}` prints).
Windows in macOS native fullscreen and windows of hidden apps don't match any `<layout>`.
+
Possible values (the same as in xref:./commands.adoc#layout[layout command]): +
+
* `h_tiles|v_tiles|h_accordion|v_accordion` - Match both tiling layout and orientation
* `tiles|accordion` - Match tiling layout regardless of orientation
* `horizontal|vertical` - Match orientation regardless of tiling layout
* `tiling|floating` - Match all tiling or all floating windows
