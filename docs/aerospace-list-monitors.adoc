...
%{all}:: A special variable which expands to all the variables listed above except for `%{right-padding}`, `%{newline}` and `%{tab}`.
Requires `--json` flag.
Useful for exploring the available variables
