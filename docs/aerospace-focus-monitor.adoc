...
The focus enters the target monitor from the opposite side, so `focus-monitor right` focuses the left-most window, `focus-monitor up` focuses the bottom-most window, etc.
Similar to `aerospace-focus`, floating windows are perceived as part of xref:guide.adoc#tree[the tree]

(next|prev)::
Focus next|prev monitor in order they appear in tray icon.
The most recently focused window of the target monitor is focused

<monitor-pattern>...::
Find the first matching monitor and focus its most recently focused window.
