...
Under the hood, the config is represented as a recursive data structure of maps, arrays, strings, and integers.

Printing without the `--json` or `--keys` flag is supported only for scalar types (strings and integers) and arrays of scalar types.
Printing other complicated objects requires the `--json` or `--keys` flag.

// =========================================================== Options
include::util/conditional-options-header.adoc[]

-h, --help:: Print help
--get <name>:: Get the value for a given key. You can inspect the available keys with `--major-keys` or `--all-keys`
--major-keys:: Print major keys
--all-keys:: Print all available keys recursively
--json:: Print result in JSON format
--keys:: Print the keys of a complicated object (map or array)
--config-path:: Print the absolute path to the loaded config
