...
--fail-if-fullscreen::
Exit with non-zero code if the target window is in fullscreen mode (as set by the `fullscreen` command).
...
+
Instead, `window 2` would be moved to the monitor above the current one.
