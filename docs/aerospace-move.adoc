...
[verse]
// tag::synopsis[]
aerospace move [-h|--help] [--window-id <window-id>] [--floating-pixels <pixels>]
...
If the window is floating, move it in the given direction by the specified number of `<pixels>`. +
The window doesn't cross the edges of its monitor. +
The option has no effect on tiling windows. +
The default is: 10% of the monitor width for `left` and `right`, 10% of the monitor height for `up` and `down`
