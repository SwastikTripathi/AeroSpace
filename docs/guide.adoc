...
The workspaces that `persistent-workspaces` doesn't keep alive are called <<phantom-workspaces,phantom workspaces>>.
...
The created workspace is always <<phantom-workspaces,phantom>>.
...
Workspaces that aren't listed in the `persistent-workspaces` config option are called *phantom* workspaces.
A phantom workspace exists only as long as you use it.
It's destroyed once it contains no windows and it's neither visible nor focused.

If you keep a list of `persistent-workspaces`, the most common way to run into a phantom workspace is to connect a new monitor.
If you have no workspace assigned to the newly connected monitor, AeroSpace creates a phantom workspace for the monitor (see <<workspace-lifecycle>>).
Such a workspace is easy to recognize: it has a numeric name that you have never configured, e.g. `10`.

AeroSpace doesn't move one of your existing workspaces to the new monitor on purpose.
Which particular workspace should be moved? There is no good answer to this question.
The workspace to monitor assignment is supposed to be explicit and predictable:
if you didn't move the workspace to another monitor yourself then it shouldn't move.

If you don't want to see a phantom workspace on the new monitor,
move one of your own workspaces to the monitor with xref:commands.adoc#move-workspace-to-monitor[move-workspace-to-monitor],
or <<assign-workspaces-to-monitors,assign a workspace to the monitor>> upfront.
The phantom workspace then disappears by itself.

In <<config-version-2,`config-version = 2`>>, the workspace that you assign must also be listed in `persistent-workspaces`.
Otherwise, it's destroyed while it's empty, so it doesn't exist by the time you connect the monitor,
and AeroSpace creates a phantom workspace for the monitor anyway.
(In `config-version = 1`, `workspace-to-monitor-force-assignment` makes the workspace persistent automatically)

[#make-all-workspaces-phantom]
==== Make all workspaces phantom

If you prefer workspaces to be fully dynamic (as they are in i3), assign an empty array to `persistent-workspaces`:
...
Now workspaces are created when you switch to them, and they are destroyed as soon as you leave them empty.
The menu of the AeroSpace tray icon, xref:commands.adoc#list-workspaces[list-workspaces] and
xref:commands.adoc#workspace[`workspace (next|prev)`] only see the workspaces that are visible or contain windows.

Keep in mind that a destroyed workspace doesn't preserve its monitor assignment and the layout of its <<tree,root container>>.

xref:./goodies.adoc#i3-like-config[i3 like config] is a complete config sample that makes all workspaces phantom.
...
If a workspace that you have never configured shows up on a newly connected monitor, see <<phantom-workspaces>>.
