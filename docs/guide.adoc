...
* The order of `persistent-workspaces` defines the order in which the workspaces are presented to you:
in the menu bar, in xref:./commands.adoc#list-workspaces[list-workspaces], and in xref:./commands.adoc#workspace[workspace next/prev].
The workspaces that are not listed in `persistent-workspaces` go last in alphabetical order
(In `config-version = 1`, all workspaces are ordered alphabetically, because the inferred fallback value of `persistent-workspaces` has no meaningful order)
