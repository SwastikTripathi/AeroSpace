...
Contrary to i3, the `focus` command doesn't have a separate argument to focus floating windows.
From the perspective of the `focus` command, floating windows are part of xref:guide.adoc#tree[the tree].
...
This behavior can be disabled with the `--ignore-floating` flag.

`focus child|parent` isn't supported because the necessity of this operation is questionable.
...
--window-id <window-id>::
Focus the window with the specified `<window-id>`
...
--fail-if-fullscreen::
Exit with non-zero code if the current target window is in fullscreen mode (as set by the `fullscreen` command).
