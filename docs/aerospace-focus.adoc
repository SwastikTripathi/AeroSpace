...
`all-monitors-outer-frame` lets the focus cross monitors.
The focus enters the target monitor from the opposite side, so `focus right` focuses the left-most window of the monitor to the right
