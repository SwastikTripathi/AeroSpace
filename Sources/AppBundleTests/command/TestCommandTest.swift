...
final class TestCommandTest: XCTestCase {
    ...
    func testExec() async {
        ...
        assertEquals(
            await parseCommand("test %{window-parent-container-orientation} = horizontal").cmdOrDie.run(.defaultEnv, .emptyStdin),
            CmdResult(stdout: [], stderr: [], exitCode: Int32ExitCode(rawValue: 0)),
        )

        assertEquals(
            await parseCommand("test %{workspace-root-container-orientation} = vertical").cmdOrDie.run(.defaultEnv, .emptyStdin),
            CmdResult(stdout: [], stderr: [], exitCode: Int32ExitCode(rawValue: 1)),
        )
