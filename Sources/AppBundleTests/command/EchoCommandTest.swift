...
final class EchoCommandTest: XCTestCase {
    ...
    func testParse() {
        ...
        testParseCommandFail("echo -- %{foo}", msg: "ERROR: Can't parse 'foo'.\n       Possible values: (window-id|window-is-fullscreen|window-title|window-layout|window-parent-container-layout|workspace|workspace-is-focused|workspace-is-visible|workspace-root-container-layout|app-bundle-id|app-name|app-pid|app-exec-path|app-bundle-path|monitor-id|monitor-appkit-nsscreen-screens-id|monitor-name|monitor-is-main|right-padding|newline|tab|all)", exitCode: 2)
    ...
    func testRunAllInterpolationFails() async {
        assertEquals(Workspace.get(byName: name).focusWorkspace(), true)
        let result = await parseCommand("echo -- '%{all}'").cmdOrDie.run(.defaultEnv, .emptyStdin)
        assertEquals(result.exitCode.rawValue, 2)
        assertEquals(result.stdout, [])
        assertEquals(result.stderr, ["'all' interpolation variable cannot be expanded"])
    }
