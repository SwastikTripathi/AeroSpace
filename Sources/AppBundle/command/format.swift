...
struct WindowWithPrefetchedTitle {
    ...
    static func resolveWindow(_ window: Window, for format: [InterToken<InterVar>], forceFetchTitle: Bool = false, _ cm: CancellationMode) async throws -> Self {
        let needsTitle = forceFetchTitle || format.contains { $0 == .interVar(.formatVar(.window(.windowTitle))) }
